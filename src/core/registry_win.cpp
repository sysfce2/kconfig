...
{
    DWORD nbytes = 0;
    DWORD err = RegGetValueW(key, nullptr, name, RRF_RT_REG_EXPAND_SZ | RRF_RT_REG_SZ, nullptr, nullptr, &nbytes);
    ...
    err = RegGetValueW(key, nullptr, name, RRF_RT_REG_EXPAND_SZ | RRF_RT_REG_SZ, nullptr, &result[0], &nbytes);
    if (err != ERROR_SUCCESS) {
...
void parseRegValues(const QString &regKey, const QString &groupName, KEntryMap &entryMap, bool userRegistry)
{
    ...
    HKEY key;
    if (RegOpenKeyExW(userRegistry ? HKEY_CURRENT_USER : HKEY_LOCAL_MACHINE, (wchar_t *)regKey.utf16(), 0, KEY_ENUMERATE_SUB_KEYS | KEY_READ, &key)
        != ERROR_SUCCESS) {
        return;
    }
    DWORD values = 0, maxNameLen = 0;

    DWORD err = RegQueryInfoKey(key, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, &values, &maxNameLen, nullptr, nullptr, nullptr);
    ...
    for (DWORD i = 0; i < values; i++) {
        ...
        err = RegEnumValueW(key, i, name, &nameLen, nullptr, nullptr, nullptr, nullptr);

        if (err != ERROR_SUCCESS) {
            qCWarning(KCONFIG_CORE_LOG) << "Failed to enum value " << i << "of" << regKey << groupName << *name << "err:" << err << values << maxNameLen
                                        << nameLen;
            continue;
        }
...
void parseRegSubkeys(const QString &regKey, KEntryMap &entryMap, bool userRegistry)
{
    ...
    HKEY key;
    if (RegOpenKeyEx(userRegistry ? HKEY_CURRENT_USER : HKEY_LOCAL_MACHINE, (wchar_t *)regKey.utf16(), 0, KEY_ENUMERATE_SUB_KEYS | KEY_READ, &key)
        != ERROR_SUCCESS) {
        return;
    }

    DWORD subKeys = 0, maxSubKeyLen = 0;
    DWORD err = RegQueryInfoKey(key, nullptr, nullptr, nullptr, &subKeys, &maxSubKeyLen, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr);
    ...
    for (DWORD i = 0; i < subKeys; i++) {
        ...
        err = RegEnumKeyEx(key, i, subKey, &keyLen, nullptr, nullptr, nullptr, nullptr);
        if (err != ERROR_SUCCESS) {
